...
__global__ void expectation(cudaTextureObject_t img,
                            const uint nchans, 
                            const uint width, 
                            const uint height,
                            hNODE* heap, 
                            ...
                            const int invwt)
{
    ...
    const uint boffset = blockDim.x*blockIdx.x;
        ...
        const int innumk, int* outnumk, double compactness)
{
	...
	maxoptin = 0.8*(float)maxoptin;
	if(DEBUG_LEVEL == 1)
	{
    	cout << "optin " << maxoptin/1024 << endl;
	}
    cudaFuncSetAttribute(expectation, cudaFuncAttributeMaxDynamicSharedMemorySize, 
            maxoptin);
    SAFE_CALL();

	cudaFuncSetAttribute(expectation,
    ...
    BENCHMARK_START(expectation)
    auto bench_start = std::chrono::high_resolution_clock::now();
    expectation<<<blocks, TOP_K, heap_size*sizeof(hNODE)>>>(d_img, nchans, width,
            height, d_heap,heap_size, d_heap_lens,toBePushed, 
            d_labels, d_xs, d_ys, d_sizes, d_colors, d_pixelCount, numk, invwt);
    cudaDeviceSynchronize();
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        fprintf(stderr, "Expectation kernel failed'%s'\n", 
